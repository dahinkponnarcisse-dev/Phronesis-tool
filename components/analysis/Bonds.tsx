...
const Bonds: React.FC = () => {
  const [ticker, setTicker] = useState('GOVT'); // Example issuer/symbol
  ...
  const generatePrompt = () => {
    return `Explain how to value a bond based on its coupon rate, face value, years to maturity, and the current market interest rate. Then provide a sample calculation for a bond from an issuer like ${ticker} with a face value of $${inputs.faceValue}, a coupon rate of ${inputs.couponRate}%, maturing in ${inputs.maturityYears} years, assuming a market interest rate (discount rate) of ${inputs.marketRate}%. Format as simple text. Do not use markdown.`;
  };
  ...
  return (
    <Card title="AI-Powered Bond Analysis">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4">
           <h4 className="text-lg font-medium">Bond Valuation Model</h4>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Issuer/Symbol</label>
        ...
        <div className="lg:col-span-2">
            ...
            <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border dark:border-gray-700 min-h-[300px]">
                {isLoading ? (
                    ...
                    <p className="text-text-secondary dark:text-dark-text-secondary whitespace-pre-wrap">{analysis || 'Enter bond parameters and click "Generate Analysis" to see the results.'}</p>
                )}
            </div>
        </div>
      </div>
